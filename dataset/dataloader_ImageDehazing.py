...
class Dataset_RESIDE(data.Dataset):
    """
    适用于RESIDE的ITS和OTS的数据集读取，有雾图片和清晰图片名称不对应，并且清晰图像需要进行中心裁剪才和有雾图像尺寸一致
    """
    def __init__(self, path, img_size, if_flip, if_crop, crop_size, trans_hazy=None, trans_gt=None):
        super(Dataset_RESIDE, self).__init__()
    ...
    def __getitem__(self, index):
        data_hazy = Image.open(self.haze_imgs[index]).convert('RGB')
        hazy_img = self.haze_imgs[index]
        id = hazy_img.split('/')[-1].split('_')[0]
        clear_name = id + hazy_img[-4:]
        data_gt = Image.open(os.path.join(self.clear_dir, clear_name)).convert('RGB')
...
class Dataset_RESIDE_val_split(data.Dataset):
    def __init__(self, pred_path, label_path, img_size, trans_hazy=None, trans_gt=None):
        super(Dataset_RESIDE_val_split, self).__init__()
...
def get_train_val_loader(dataset, img_h, img_w, train_batch_size, num_workers, if_flip, if_crop, crop_h, crop_w):
    supported_dataset = {
        "OHAZE": Dataset_OHAZE,
        "OHAZE_val10": Dataset_OHAZE,
        "DenseHaze": Dataset_OHAZE,
        "NHHAZE": Dataset_OHAZE,
        "ITS": Dataset_RESIDE,
        "OTS": Dataset_RESIDE,
        "4KDehazing": Dataset_OHAZE,
        "NYU_Seg_Haze": Dataset_OHAZE,
        # Nighttime Dehazing
        "NHR": Dataset_OHAZE,
        "NHM": Dataset_OHAZE,
        "NHCL": Dataset_OHAZE,
        ...
        "SateHaze1k_thick": Dataset_OHAZE,
        "SateHaze1k_thin": Dataset_OHAZE,
        "SateHaze1k_moderate": Dataset_OHAZE,
        "LHID": Dataset_RESIDE,
        "DHID": Dataset_RESIDE,
        "RICE1": Dataset_OHAZE,
        "RICE2": Dataset_OHAZE,
...
def get_split_val_loader(dataset_name, batch_size, num_workers, pred_path, label_path, img_h, img_w):
    try:
        supported_dataset = {
            "OHAZE": Dataset_OHAZE_val_split,
            "ITS": Dataset_RESIDE_val_split,
            "OTS": Dataset_RESIDE_val_split,
            "4KDehazing": Dataset_OHAZE_val_split,
            "NYU_Seg_Haze": Dataset_OHAZE_val_split,
            # Nighttime Dehazing
            ...
            # Remote Sensing Dehazing
            "SateHaze1k_thick": Dataset_OHAZE_val_split,
            "SateHaze1k_thin": Dataset_OHAZE_val_split,
            "SateHaze1k_moderate": Dataset_OHAZE_val_split,
            "LHID": Dataset_RESIDE_val_split,
            "DHID": Dataset_RESIDE_val_split,
            "RICE1": Dataset_OHAZE_val_split,
            "RICE2": Dataset_OHAZE_val_split,
            "RSID": Dataset_OHAZE_val_split
        }
    except:
