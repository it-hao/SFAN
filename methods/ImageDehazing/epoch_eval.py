...
def eval_SISO(val_loader, network, device, save_dir, if_eval_best, if_eval_last, network_name, dataset_name, multi_supervised=False):
    ...
    def cell(val_loader, network, device, save_dir, pth_name, metric_name, save_type):
        ...
        ssim, psnr = eval_SISO_cell(val_loader=val_loader, network=network, device=device, save_dir=save_dir,
                                    if_save_cat=False, save_type=save_type, multi_supervised=multi_supervised)
...
def eval_SISO_cell(val_loader, network, device, save_dir, if_save_cat, save_type, multi_supervised=False):
    network.eval()
    with torch.no_grad():
        num_samples = 0
        total_ssim = 0
